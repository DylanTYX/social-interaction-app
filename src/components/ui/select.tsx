...
/**
 * `w-full`, not shadcn's default `w-fit`.
 *
 * `w-fit` suits a toolbar select that should hug its label. Every Select in
 * this app is in a form grid beside an `Input`, which is `w-full` — so the two
 * could never line up, and a "Communication style" select rendered at half the
 * width of the "Years of experience" input beside it.
 *
 * The three call sites that genuinely want a fixed width already pass one
 * (`sessions/page.tsx`, `code-input.tsx`), so they are unaffected.
 */
