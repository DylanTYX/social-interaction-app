-- ---------------------------------------------------------------------------
-- Document metadata: truncation, resume labels, and a missing trigger
-- ---------------------------------------------------------------------------
--
...
--
-- `variant` and `notes` are for the resume library only, and are deliberately
-- library-only: neither reaches the interviewer's prompt. `variant` is the resume
...
-- file. There is no `company` equivalent, because a resume does not have one.
...
-- for every resume ever saved. Nothing noticed while the table had no update path
