...
```
npm test          649 tests, no network         ← layers 1-3
...
ESM-only `@exodus/bytes`. Recorded rather than hidden. The 649 passing tests are
layers 1 and 2.
...
Worth stating for a reader who counts tests: 649 is not a quality claim. Two
