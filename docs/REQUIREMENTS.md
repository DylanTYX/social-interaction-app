...
| F9  | **The score steers the next question**                           | `decision-engine.ts` → `steering-block.ts`, sent after the exchange ([DESIGN-DECISIONS.md](DESIGN-DECISIONS.md) §17) | `decision-engine.test.ts`, `steering-block.test.ts`; [PERSONA-EVAL.md](PERSONA-EVAL.md) — pivot compliance 0/10 before the reorder, 10/10 after |
...
| F15a | An interviewer speaks English with the accent their nationality suggests | `persona-voice.ts`, `speech-voices.ts` | `persona-voice.test.ts`, `speech-queue.test.ts`, UAT V7-V10 |
...
| F22 | Progress over time, and a suggestion for what to practise next        | `dashboard/analytics`, `progress-insights.ts`, `api/analytics/rubric`, `recommendations.ts` | `session-stats.test.ts`, `progress-insights.test.ts`, `api/analytics/rubric/route.test.ts` |
...
| F25 | Export all data as JSON                              | `api/me/export`                        | UAT A3                     |
| F26 | Delete interview history from settings; account rows cascade on auth deletion (no UI path) | `dashboard/settings`, FK cascade       | UAT A4                     |
...
| N6  | **Secrets must not reach the browser**                                            | Azure keys server-side; browser gets a short-lived token. A lint rule bars `@/lib/pricing`, `@/lib/usage-summary` and `@/eval` from `src/app` and `src/components`, with one carve-out for route handlers, which never reach a bundle | `eslint.config.mjs`; CI runs lint                                  |
...
| E1b | **An unverified accent must never reach a user**                    | Azure has no `en-CN`-style voice for 11 of the 18 nationalities in the persona randomiser, so those need a native-locale voice reading English. Every one was auditioned by ear. Rejected voices stay in the table as `verified: false` with the reason recorded and can never resolve. Brazilian was re-auditioned across every voice its locale offers and uses the voices chosen that way; Japanese and Vietnamese were removed from the accent map and the randomiser, and a persona that still holds one falls back to neutral English with the reason shown. Tests enforce that no rejected voice resolves and that a multilingual voice needs an audition to be listed. Evidence: [artifacts/voice-audition.md](artifacts/voice-audition.md), [artifacts/voice-audition-explore.md](artifacts/voice-audition-explore.md) |
